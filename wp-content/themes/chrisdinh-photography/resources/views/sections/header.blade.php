...
<header class="banner relative">
    ...
    @if ($menuItems)
        ...
        <div id="menuOverlay" class="fixed inset-0 bg-neutral-900 opacity-0 hidden transition-opacity duration-300 ease-in-out z-20 bg-cover bg-center">
            <div class="flex h-full flex-col-reverse lg:flex-row justify-between items-center">
                <nav class="w-full flex flex-col justify-end items-end p-4 text-white lg:w-1/2 lg:justify-start lg:items-start lg:pl-48" aria-label="Primary Navigation">
                    <ul class="flex flex-col w-full items-end lg:items-start">
                        @foreach ($menuItems as $menuItem)
                            @php
                                // Fetch the ACF field group
                                $navigation_image = get_field('navigation__image', $menuItem);

                                // Access the subfields
                ...
                <div class="w-full flex flex-col justify-start items-start p-4 pt-24 text-white whitespace-nowrap lg:w-1/2 lg:pr-48">
